...
class SE3Node(Node):
    def __init__(self):
        ...
        try:
            config_path = os.path.join(os.path.dirname(__file__), "../configs/xi35.yaml")
        ...
        self.sim_step_client = self.create_client(StepSim, '/sim_node/step_sim')
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            if future.result() is not None:
                self.state = future.result().state
            else:
                self.get_logger().error('Exception while calling service. Shutting down.')
                break
            ...
            current_state_array = np.array(self.state.state)
